...
describe('JWTValidator', function() {
   ...
   it('validates a non-bearer token', function() {
      ...
      expect(_.chain(validation.errors).invoke('toResponseObject').map(_.partial(_.omit, _, 'id')).value()).to.eql([
         { title: 'Invalid authorization token', detail: 'Token has been revoked' },
      ]);
   });

   it('has correct error message when bearer token comes from non-header source', function() {
      var validator = new Validator(publicKey),
          token = makeToken({}, { aud: 'MyAPI', iss: 'MyIssuer', jti: 'id-321' }),
          validation;
      ...
      expect(validator.validate('Bearer ' + token.string, true)).to.eql({ errors: [], token: token.obj });

      token = makeToken({}, { aud: 'MyAPI', iss: 'MyIssuer', jti: 'id-123' });
      validation = validator.validate(token.string, true); // report that it's bearer format, but don't send it in correct format

      expect(validation.token).to.be(undefined);
      expect(validation.errors).to.be.an('array');
      expect(_.chain(validation.errors).invoke('toResponseObject').map(_.partial(_.omit, _, 'id')).value()).to.eql([
         { title: 'Bearer token not in correct format' },
      ]);
   });
